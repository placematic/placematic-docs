...
<Tip>
Wire those three assertions into your test suite, not your QA checklist. A refactor that drops `vehicle[height]` from a params dict is invisible in code review and obvious in a failing test.
</Tip>
...
**Google Maps Platform** added Large Vehicle Routing in August 2026 — dimensions, gross weight, axle count and hazmat classes, for the 48 contiguous United States. For a US-domestic fleet running online dispatch it is now a real option and you should evaluate it rather than dismiss it. What it does not do, per Google's own documentation: truck toll prices, speed limits, radioactive cargo. No coverage beyond the contiguous 48. No offline navigation. And it has no equivalent to Route Matching, which means no defensible jurisdiction-mile reconstruction. If your platform crosses a border, quotes freight, or answers to an auditor, those are the reasons — not the absence of a truck profile. Keep Google if your product also does consumer place discovery.
...
Also: [Reverse Geocoding](/guides/reverse-geocoding) · [HERE SDK](/guides/here-sdk) · [HERE Pricing Explained](/getting-started/here-pricing-explained)
...
See the packaged solution for your industry: [Fleet & ELD](https://placematic.com/solutions/fleet-eld/)
