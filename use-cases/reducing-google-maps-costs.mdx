...
| Which Google SKU your truck traffic lands on | Console — LVR does not bill at Essentials |
...
**Truck routing — and this became a cost argument in August 2026.** On HERE a truck route bills as a routing request: same rate, same free allowance as a car route. Google's Large Vehicle Routing requires the traffic-aware optimal routing preference, which bills above its Essentials rate. Describing your vehicle now costs more per call on Google and nothing extra on HERE. It remains a capability argument as well — toll cost, speed limits and coverage beyond the contiguous 48 are documented gaps — but the meter difference is real and belongs in the model above.
