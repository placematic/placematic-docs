...
A single shipment may carry several cargo types and one tunnel category. Do not map an internal `is_hazmat` boolean onto either. Read the definitions in the [Routing API v8 reference](https://docs.here.com/routing/reference/routing-api-v8-calculateroutes) and map your material classifications deliberately.
</Info>
...
**Google Maps Platform** added hazardous goods classification to Large Vehicle Routing in August 2026, for the 48 contiguous United States. Its documentation lists routing for radioactive materials as unsupported, and it disclaims reliance — the response can flag that restrictions were partially ignored, and drivers are told not to rely solely on the route being safe or legal. For a US domestic placarded haul outside class 7, it is now a candidate worth evaluating. For radioactive cargo, cross-border movement, or any operation where the route is the compliance artifact, the disclaimer alone should decide it. Verify the class mapping against Google's current reference rather than assuming parity with HERE's eleven cargo types.

**Mapbox** does not express hazmat constraints.
...
- [Routing API v8 reference](https://docs.here.com/routing/reference/routing-api-v8-calculateroutes) — `shippedHazardousGoods`, tunnel category, truck parameters
