...
<Info>
`shippedHazardousGoods` is a **classed enum**, not a boolean. HERE's specification includes class values such as `B`, `C`, `D`, and `E`, and which classes are supported depends on transport mode. Tunnel category is a separate parameter.

Do not map an internal `is_hazmat` boolean onto this. Read the class definitions in the [Routing API v8 reference](https://www.here.com/docs/bundle/routing-api-v8-api-reference/page/index.html) and map your material classifications deliberately.
</Info>
...
**`weightLimit` is deprecated** in the current v8 specification in favour of `grossWeight`. If you are reading an older tutorial, you are reading a deprecated parameter.
...
**Mapping an internal `is_hazmat` boolean onto a classed enum.** Read the class definitions.
