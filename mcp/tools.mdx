...
<Warning>
Toll estimates depend on the same profile. Axle count in particular changes the rate: the same route returned USD 30.88 with height and weight only, and USD 55.00 once length and a 5-axle configuration were supplied. If you are calling the underlying REST endpoint directly, nothing will warn you about either.
</Warning>
