...
**No `search_nearby` or `search_address` yet.** Both are planned. Neither is exposed today, and an
agent asking for a radius search or ranked address candidates will not find a tool for it.

**No dataset version in the response.** Responses do not currently state which dataset version
answered. This is in development in the REST layer; when it ships it will surface here.

**No per-key rate limits or request logs.** During beta, access is by a shared credential and there
is no per-caller metering. If you need usage attribution before general availability, tell us when
you request access.
