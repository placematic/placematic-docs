...
<Warning>
The facade is where truck constraints get lost, and August 2026 made this harder to catch, not easier.

An interface derived from Google before Large Vehicle Routing had no field for vehicle height at all — obviously incomplete the moment you added HERE. An interface derived from Google's large vehicle profile has height, weight and axle count, and looks complete. The fields it cannot carry — weight per axle group, ADR tunnel category, toll cost — are exactly the ones that determine legality where regulation is tightest.

Design the interface from the richer constraint set, and assert field-level parity in tests rather than trusting that a provider responded.
</Warning>
...
**Designing the facade from the narrower constraint set.** The missing fields are silent, and since August 2026 the facade looks complete while missing them.
