...
- **Truck routing.** A second provider does not carry HERE's full constraint set — and since August 2026 this got harder to notice, not easier. A fallback to car routing produces an obviously wrong route. A fallback to a partial truck profile produces a plausible one. If legality depends on axle-group weight, tunnel category, or a cargo class the fallback cannot express, the route looks correct, passes review, and is illegal. **The second case is worse than the first.**
...
<Warning>
Failing over truck routing to a provider with a narrower constraint set is the single most dangerous fallback in this domain — and a partially-constrained fallback is more dangerous than an unconstrained one, because it fails silently.

Assert field-level parity in the facade, not merely that a provider responded. If the secondary cannot carry every constraint the primary was given, refuse to route. A dispatcher calling operations is a better outcome than a driver following a route through a low bridge.
</Warning>
...
**Failing over truck routing to a provider with a narrower constraint set.**
...
- [ ] No failover from constrained truck routing to a provider that cannot carry every constraint; parity asserted per field, not per response
