...
**API keys authenticate the application, not the user.** They belong in server-side environment variables. See [Authentication](/start-here/authentication) for the full treatment, including when you need OAuth instead.
...
resp = requests.get(
    ...
    headers={"x-api-key": os.environ["PLACEMATIC_KEY"]},
    params={
        "transportMode": "truck",
...
const resp = await fetch(
  ...
  { headers: { "x-api-key": process.env.PLACEMATIC_KEY } }
);
...
- **Log the HTTP status, not just the body.** `403` and `429` mean entirely different things and require different responses. See [Authentication](/start-here/authentication).
...
Different auth mechanisms for different APIs and different security postures. Covered in [Authentication](/start-here/authentication).
