...
**Every API is billed independently.** There is no bundle. A route calculation, a geocode, and a tile request are three different meters. Cost forecasting means forecasting each call type separately — see [HERE Pricing Explained](/start-here/here-pricing-explained).
...
Depends on volume and procurement tolerance. Direct contracts typically involve minimum volume commitments and a sales cycle. Gold Partner pricing tiers are not available to direct customers at standard volumes. See [HERE Pricing Explained](/start-here/here-pricing-explained) for the honest version.
...
Also relevant: [Authentication](/start-here/authentication) · [Truck Routing Guide](/guides/truck-routing) · [Distance Matrix](/guides/distance-matrix) · [Migrating from Google Maps](/guides/google-migration)
