...
| Truck routing | Medium–high | New surface since Aug 2026. Unit conversion plus constraint-tail mapping |
...
**Vehicle units differ.** HERE's `vehicle[height]` is centimetres. Google's large vehicle fields are named in millimetres — `totalHeightMm`. A factor-of-ten error produces either a vehicle that routes nowhere or a 41-centimetre vehicle that routes everywhere and returns `200`. Convert at the adapter, assert the converted value in a test.
