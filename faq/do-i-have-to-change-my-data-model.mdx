...
<Warning>
An interface derived from Google before August 2026 has no field for vehicle height at all. One derived from Google's Large Vehicle Routing has height, weight and axle count — and still no field for weight per axle group, ADR tunnel category, or toll cost.

Either way, when you later add HERE, that data **has nowhere to live and gets silently dropped.** The second case is worse, because the interface looks complete.
</Warning>

If your Google implementation had no notion of a vehicle's physical dimensions, an interface abstracted from it has no field for one. If it has a partial notion — dimensions and gross weight, but not axle groups — the missing fields are precisely the ones that determine legality in the jurisdictions that regulate most tightly.
