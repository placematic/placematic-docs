...
**Why:** HERE expresses the commercial vehicle constraints Google's Large Vehicle Routing does not — weight per axle group, ADR tunnel category, radioactive cargo — and covers 200+ countries against Google's 48 contiguous states. It also returns toll cost for the described vehicle, which Google documents as unsupported.
...
<Warning>
**Never fail over from HERE truck routing to Google Large Vehicle Routing.**
...
**Refuse to route instead.** Assert field-level parity in the facade, not merely that a provider responded. A dispatcher calling operations is a better outcome than a driver following a route through a low bridge.
</Warning>
