...
**Short answer:** Yes. Google added commercial vehicle constraints in August 2026, for the contiguous US. It still has no equivalent to Route Matching, and no `MapVersion` in any response — which means no defensible reconstruction of driven segments and no audit artifact. For an ELD platform that is the deciding difference, and it is not a close one.
