...
- **You route commercial vehicles outside the contiguous United States, under axle-group weight regulation, or with toll cost in the dispatch decision.** Google added Large Vehicle Routing in August 2026 for the contiguous 48, so "Google can't do trucks" is no longer the argument. Coverage, toll cost, offline guidance and the constraint tail are.
...
**4b. Truck routing, if you were on Google's Large Vehicle Routing.** A separate surface with its own defect class. Map `totalHeightMm` to `vehicle[height]` in centimetres, `travelMode=TRUCK` + `routeModifiers.vehicleInfo` to `transportMode=truck` + `vehicle[...]`, and re-map the hazardous goods taxonomy deliberately. Constraints Google could not express — weight per axle group, ADR tunnel category — have to be sourced from your fleet register, not inferred from the old request payload.
...
**Vehicle units differ by a factor of ten.** HERE's `vehicle[height]` is centimetres. Google's large vehicle fields are millimetres — `totalHeightMm`. Convert at the adapter and assert the converted value in a test, not the input. A 41-centimetre truck routes everywhere and returns `200`.
